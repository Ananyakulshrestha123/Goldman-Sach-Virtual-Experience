    ...
    with open(file_path, 'rb') as f:
        ...
        for flow, packets in flow_windows.items():
            ...
            for ts, buf, src_port, dst_port in packets:
                ...
                # Add packet to the bucket
                if bucket in buckets:
                    buckets[bucket].append((timestamp, direction, packet_size))
                else:
                    buckets[bucket] = [(timestamp, direction, packet_size)]
            ...
            for bucket, packet_info in buckets.items():
                ...
                forward_packet_count = sum(1 for _, dir, _ in packet_info if dir == "Forward")
                backward_packet_count = packet_count - forward_packet_count

                forward_bytes_per_sec = total_bytes_forward / bucket if bucket > 0 else 0
                backward_bytes_per_sec = total_bytes_backward / bucket if bucket> 0 else 0
                ...
                for packet in packet_info:
                    timestamp, direction, packet_size = packet
                    print(f"Timestamp: {timestamp}\tDirection: {direction}\tPacket Size: {packet_size}")
    ...
    with open(file_path, 'rb') as f:
        ...
        for flow, packets in flow_windows.items():
            ...
            for ts, buf, src_port, dst_port in packets:
                ...
                if connection_id_to_str((ip.src, tcp.sport, ip.dst, tcp.dport)) == flow:
                    direction = 1  # Forward packet
                    forward_packets += 1
                    total_bytes_forward += packet_size
                else:
                    direction = -1  # Backward packet
                    backward_packets += 1
                    total_bytes_backward += packet_size
            ...
            for bucket, packet_info in buckets.items():
                ...
                forward_packet_count = sum(1 for _, _, direction in packet_info if direction == 1)
                backward_packet_count = sum(1 for _, _, direction in packet_info if direction == -1)
...
def analyze_pcap_file(file_path):
    ...
    with open(file_path, 'rb') as f:
        pcap = dpkt.pcap.Reader(f)
        print("Flow\tTimestamp\tDirection\tData Size")
        ...
        for ts, buf in pcap:
            ...
            if flow in flow_windows:
                flow_windows[flow].append((ts, tcp.sport, tcp.dport, len(tcp.data)))
            elif reverse_flow in flow_windows:
                flow_windows[reverse_flow].append((ts, tcp.sport, tcp.dport, len(tcp.data)))
            else:
                flow_windows[flow] = [(ts, tcp.sport, tcp.dport, len(tcp.data))]

        # Print flow windows
        for flow, packets in flow_windows.items():
            flow_info = []
            for ts, src_port, dst_port, data_size in packets:
                direction = 1 if connection_id_to_str((ip.src, tcp.sport, ip.dst, tcp.dport)) == flow else -1
                flow_info.append([ts, direction, data_size])
            print(f"{flow}\t{flow_info}")
    ...
    with open(file_path, 'rb') as f:
        ...
        for flow, packets in flow_windows.items():
            ...
            for ts, src_port, dst_port, data_size in packets:
                direction = 1 if connection_id_to_str((ip.src, tcp.sport, ip.dst, tcp.dport)) == flow else -1
                timestamp = ts - start_time  # Calculate the timestamp in seconds since the first captured packet
                flow_info.append([timestamp, direction, data_size])
            print(f"{flow}\t{flow_info}")
