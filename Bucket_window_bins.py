            ...
            for packet in packets:
                ...
                if flow not in window_buckets[window_index]:
                    window_buckets[window_index][flow] = {
                        ...
                        'forward_time': [],
                        'backward_time': [],
                        'forward_bins': [0] * math.floor(window_size / bin_size),
                        'backward_bins': [0] * math.floor(window_size / bin_size),
                        'forward_mean': 0,
                        'forward_min': 0,
                        'forward_max': 0,
                        'forward_std': 0,
                ...
                if flow_data['is_active'] == 0:
                    ...
                    if hard_timeout < (timestamp - flow_data['buffer_time']):
                        ...
                        flow_data['active'].append(flow_data['buffer_time'] - flow_data['active_start_time'])
                        flow_data['idle'].append(timestamp - flow_data['buffer_time'])
                    else:
...
#                
                ...
                if hard_timeout < window_size * (idx + 1) - flow_data['buffer_time']:
                    flow_data['active'].append(flow_data['buffer_time'] - flow_data['active_start_time'])
                    flow_data['idle'].append(window_size * (idx + 1) - flow_data['buffer_time'])
                else:
                    flow_data['active'].append(window_size * (idx + 1) - flow_data['active_start_time'])
